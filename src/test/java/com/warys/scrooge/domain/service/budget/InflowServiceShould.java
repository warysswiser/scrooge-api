...
class InflowServiceShould {
    ...
    private final InflowRepository inflowRepository = mock(InflowRepository.class);
    private final InflowService tested = new InflowService(inflowRepository);
    ...
    void get_all_inflow_for_user() {
        ...
        when(user.getId()).thenReturn(CURRENT_USER_ID);
        when(inflowRepository.findByOwnerIdAndCreationDateBetween(anyString(), any(LocalDate.class), any(LocalDate.class)))
