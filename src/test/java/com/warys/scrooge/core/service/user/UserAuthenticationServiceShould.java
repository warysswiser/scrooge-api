...
public class UserAuthenticationServiceShould {
    ...
    private static final UserCommand VALID_COMMAND = new UserBuilder().with(o -> {
        o.id = VALID_ID;
        o.email = VALID_EMAIL;
        o.password = VALID_PASSWORD;
    }).buildCommand();
    private static final User VALID_USER = new UserBuilder().with(
            o -> {
                o.id = VALID_ID;
                o.email = VALID_EMAIL;
                o.password = VALID_PASSWORD;
    ...
    public void register() throws ApiException {
        ...
        when(userRepository.insert(any(User.class))).thenReturn(VALID_USER);

        final UserCommand actual = tested.register(VALID_COMMAND);
    ...
    public void find_by_token() {
        ...
        final UserCommand userCommand = tested.findByToken(VALID_TOKEN).orElseThrow();
    ...
    public void return_empty_user_when_invalid_token_is_given() {
        Optional<UserCommand> userCommand = tested.findByToken("invalid_token");
